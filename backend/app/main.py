...
# Ruta para manejar rutas desconocidas del frontend
@app.get("/{full_path:path}", include_in_schema=False)
def spa_routes(full_path: str):
    # Si el archivo existe en static, lo servimos directamente (por ejemplo assets/*.js o *.css)
    requested_path = os.path.join(STATIC_DIR, full_path)
    if os.path.isfile(requested_path):
        return FileResponse(requested_path)

    # Si no existe el archivo, devolvemos index.html (React Router se encarga)
